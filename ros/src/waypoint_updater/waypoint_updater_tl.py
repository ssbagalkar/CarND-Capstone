...
class WaypointUpdater(object):
    ...
    def publish_waypoints(self):
		...
		if self.traffic_signal != -1:
            # on traffic light slow down the car, set velocity to 0
            end_waypoint = index_of_next_waypoint
            vel = 0.0
			set_waypoint_velocity(self, self.waypoints, end_waypoint, vel)
        else :
            end_waypoint = index_of_next_waypoint+LOOKAHEAD_WPS
			vel = 10.0 # set fixed veloctiy
			set_waypoint_velocity(self, self.waypoints, end_waypoint, vel)
